# NDA Odyssey — choix des versions A / B / C / D

**Type :** temp · **Vérité pour :** quelle version d’entente envoyer (démo partenaires).  
**Dernière MAJ :** 21 août 2026 · **Carte :** [`../README.md`](../README.md)

**Changelog** (max 5)
- 21 août 2026 — **D** = B renforcé (3(e), démo, mémorisation, non-contournement, PI) · **C** réalignée sur D.
- 21 août 2026 — versions **A** (validée) · **B** (Freemium/RevShare) · **C** (robustesse équipe).

**Complète :** [`DEMO_PATRICE_URGEL_RUSH.md`](DEMO_PATRICE_URGEL_RUSH.md).

---

## En une phrase

| Version | Fichier | Quand l’utiliser |
|---------|---------|------------------|
| **A** | [`NDA_A_VALIDEE_LEGALE.md`](NDA_A_VALIDEE_LEGALE.md) | Déjà **validée légale** · signature rapide · friction minimale |
| **B** | [`NDA_B_FREEMIUM_REVSHARE.md`](NDA_B_FREEMIUM_REVSHARE.md) | Archive de travail · Freemium/RevShare **sans** polish D |
| **D** | [`NDA_D_RENFORCE.md`](NDA_D_RENFORCE.md) | **Reco CEO** pour Patrice · B + clauses démo / exécutabilité · **revalidation avocat** |
| **C** | [`NDA_C_ROBUSTE.md`](NDA_C_ROBUSTE.md) | **D** + non-sollicitation équipe + propriété du feedback · **revalidation avocat** · plus de négociation |

Remplir les **`[CROCHETS]`** avant signature (DocuSign OK).  
**Ops :** NDA ≠ accès code source — démo UI / compte limité seulement.

---

## Différences (ce qui change vraiment)

| Élément | **A** Validée | **B** Freemium | **D** Renforcé | **C** Robuste |
|---------|---------------|----------------|----------------|---------------|
| Secret tech + Coffre / Sanctuaire / Film | Oui | Oui | Oui | Oui |
| Freemium / Héritage-Éternité / RevShare nommés | Non | **Oui** | **Oui** | **Oui** |
| §5 interdiction de copier freemium / RevShare | Non | **Oui** | **Oui** | **Oui** |
| Exclusion « généralement connu dans l’industrie » (ex-3(e)) | Oui (présente) | Oui (présente) | **Retirée** + précision combinaison | **Retirée** + précision combinaison |
| Démos / accès plateforme / comptes démo = IC | Partiel | Partiel | **Explicite** | **Explicite** |
| Infos mémorisées (pas d’échappatoire) | Non | Non | **Oui** | **Oui** |
| Non-contournement + « connaissance antérieure » | Large | Large | **Précisé** | **Précisé** |
| § PI (sans mini-plaidoirie art. LMC) | Art. cités | Art. cités | **Allégé** | **Allégé** |
| Non-sollicitation 12 mois (équipe / DA / dévs) | Non | Non | Non | **Oui** (§7) |
| Feedback = propriété Odyssey | Non | Non | Non | **Oui** (§8) |
| Durée conf. / non-contournement 3 ans | Oui | Oui | Oui (à valider avocat) | Oui (à valider avocat) |
| Signature électronique | Oui | Oui | Oui | Oui |
| Statut légal Odyssey | **Validée** | Brouillon | Brouillon jusqu’à OK avocat | Brouillon jusqu’à OK avocat |
| Friction signature gros groupe | Faible | Moyenne | Moyenne | Plus haute |

**A → B :** protection du **business model**.  
**B → D :** mêmes $ + clauses cœur **durcies** pour démo live.  
**D → C :** + protection **équipe** et **idées** données en call.

---

## Reco

1. **Cette semaine, sans attendre :** **A** si tu dois faire signer demain.  
2. **Cible Patrice :** **D** après OK avocat court (surtout non-contournement, durée, non-utilisation concurrente).  
3. **C** si l’avocat dit OK et que tu es prêt à négocier non-sollicitation / feedback.  
4. **B** : garder comme historique ; ne plus envoyer si **D** est prête.

Ne pas envoyer **B**, **D** ou **C** en disant « version déjà validée » tant que l’avocat n’a pas vu **ces** ajouts.
