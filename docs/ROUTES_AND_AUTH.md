...
| `app/api/partner/wallet/route.ts` | Snapshot wallet admin (`canViewBalance`) |
...
6. `/fr/salon` — header logo = même PNG, dropdown tenant, pas d’erreur « espace introuvable » ; **Admin** voit solde réel (pas mock) ; **Directeur** ne voit pas le bloc jetons.
