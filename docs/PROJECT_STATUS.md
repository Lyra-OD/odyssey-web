...
**Changelog** (max 5)
- 8 sept 2026 — **Accueil clarifié sans casser la DA cinéma** : la home redevient clairement **B2C**. Le hero promet plus vite ce qu’Odyssey fait réellement, le Process met l’**Entourage** au centre, et la section [`Pricing`](../src/components/Pricing.tsx) revient juste après le parcours avec la grille **B2C directe** canonique (**Héritage 179 $ · Éternité 349 $ · Légendaire 499 $**) — sans afficher `Souvenir 0 $`, qui reste réservé au freemium salon. Le `Manifesto` et [`Partnerships`](../src/components/Partnerships.tsx) ferment ensuite la page. Aucun changement de palette, de typo, de vidéo ni d’animation ; uniquement copy FR/EN + réordonnancement de composants déjà existants.
- 8 sept 2026 — **Page Partenaires réécrite sans changer le design** : le discours public pour maisons funéraires quitte le lexique trop SaaS (`acquisition virale`, `rétention`, `socio-financement` en façade) au profit d'un positionnement plus crédible et haut de gamme : paradoxe terrain, promesse opératoire `moins de 60 secondes en salon, zéro montage`, rayonnement digne, générosité partagée et bloc confiance explicite (Loi 25, contrôle famille, portail partenaire). Copy écran mise à jour en FR/EN.
- 8 sept 2026 — **Dépôt de mot texte invité impossible** (« Impossible d'ajouter ce souvenir pour le moment », 100 % des cas) : le bucket Storage `user-assets` a une allowlist MIME qui n'a jamais inclus `text/plain` — contrairement à voix/vidéo invité qui avaient déjà un garde-fou auto-réparant (`ensureUserAssetsAllowsGuestVoiceAudio` / `...Video`), le mot texte n'en avait pas. Corrigé avec le même patron : [`ensureUserAssetsAllowsGuestMessage.ts`](../src/lib/media/ensureUserAssetsAllowsGuestMessage.ts) étend l'allowlist avant l'upload dans `POST /api/contribute/[token]/deposit`, + retrait du `; charset=utf-8` en trop dans le `Content-Type` envoyé au Storage (ne correspondait à aucune entrée d'allowlist). Diagnostic écarté au passage : `tenant_id` (4 requêtes SQL prod, 0 ligne orpheline — la chaîne tenant est saine).
