...
**Dernière MAJ :** 15 sept 2026

## Inventaire

| Fichier | Statut | Slots dynamiques |
|---------|--------|------------------|
| [`intro.json`](intro.json) | Figé UI | `{{displayName}}` · `{{birthYear}} - {{deathYear}}` |
...
- **Text-936** (Playfair) → nom famille `displayName`
- **Text-MN2** (tracking large) → années seules `"YYYY - YYYY"`
