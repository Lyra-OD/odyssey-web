...
export function LueurNode({
  ...
  useFrame(({ clock }) => {
    ...
    // Double sinusoïde = respiration organique (pas mécanique)
    const breath =
      0.55 +
      0.3 * Math.sin(t) +
      0.15 * Math.sin(t * 0.37 + 1.2);
    const mat = matRef.current ?? material;
    mat.uniforms.uSize.value = cfg.size * (0.88 + 0.22 * breath);
    mat.uniforms.uGlow.value = 0.65 + 0.45 * breath * (1 + cfg.pulse);
  });
