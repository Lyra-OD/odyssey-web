...
/** D ≈ first 45 % of draw · E until ~88 % · F last ~12 % + idle */
const D_END = 0.45;
const E_END = 0.88;
...
export function resolveDrawPhase(revealT: number): DrawPhase {
  ...
  const whisperBlend = smoothstep(E_END, 1, localU);
  const whisper =
  ...
  const emphasis =
    t >= 1
      ...
      : beat === "F"
        ? emphasisRamp * (1 - whisperBlend * 0.65) + whisper
        : emphasisRamp;

  const proximity = localU >= 0.72 || t >= 1;

  return { beat, emphasis, whisper, proximity, localU };
}
