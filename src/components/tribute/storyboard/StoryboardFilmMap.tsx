...
export type StoryboardFilmMapWatchSession = {
  label: string;
  aria: string;
};
...
type Props = {
  ...
  watchSession?: StoryboardFilmMapWatchSession;
  onWatchSession?: () => void;
};
...
export const StoryboardFilmMap = memo(function StoryboardFilmMap({
  ...
  const showWatch = Boolean(watchSession && onWatchSession);

  if (segments.length === 0 && !showWatch) return null;
  ...
  return (
    ...
    >
      {showWatch && watchSession && onWatchSession ? (
        <div className="mb-2 flex justify-end">
          <button
            type="button"
            onClick={onWatchSession}
            aria-label={watchSession.aria}
            className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-3.5 py-1.5 transition-colors hover:border-white/25 hover:bg-white/[0.08] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/25"
          >
            <Play
              className="h-3 w-3 text-neutral-200"
              strokeWidth={1.2}
              aria-hidden
            />
            <span className="font-sans text-xs uppercase tracking-[0.2em] text-neutral-300">
              {watchSession.label}
            </span>
          </button>
        </div>
